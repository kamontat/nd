...
${HELP_CONFIGURATION(self.name)}`);
            ...
            .sub(
                SubCommand.build('set', true, ({ value, apis }) => {
                    ...
                    if (!parsed)
                        throw Exception.build(ERR_CFG).description('cannot parse any key or value from input string');

                    parsed.forEach(c => {
                        try {
                            config.set(c.key as unknown, c.value as unknown);
                            LoggerService.console.log(
                                `  - set ${Colorize.key(c.key)} to ${Colorize.value(c.value.toString())}`,
                            );
                        } catch (e) {
