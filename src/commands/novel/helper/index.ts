...
export const htmlConfigBuilder = (type: 'novel' | 'chapter', secure: Security, novel: Novel, chapter?: Chapter) => {
    const htmlConfig = {
        auth: {
            username: Optional.of<unknown, string>(secure.response)
                .transform(r => r.username)
                .or(''),
            name: config.get('auth.name') as string,
            expireat: Optional.of<unknown, number>(secure.response)
                .transform(r => r.expire)
                .or(0),
            issueat: Optional.of<unknown, number>(secure.response)
                .transform(r => r.issue)
                .or(0),
        },
        novel: {
